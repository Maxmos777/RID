...
| [ADR-009](ADR-009-langflow-database-integration.md) | Integração Django ↔ Langflow: Base de Dados Partilhada e Multi-tenancy via Workspaces | ✅ Accepted | 2026-04-04 | P2 | `docker-compose.yml`, `apps/tenants/models.py`, `api/services/langflow_workspace.py`, `apps/tenants/langflow_pg_bootstrap.py` |
...
**Score de compliance:** 8/11 (73%) — Score P0: 2/2 (✅ COMPLETO). Testes pytest em `test_architecture.py` cobrem ADR-001 a ADR-006 (15 testes) e ADR-009 (4 testes, validado em ambiente). ADR-007, ADR-008, ADR-010 e ADR-011 ainda não têm testes de compliance automatizados nesse ficheiro (ver secções Compliance em cada ADR).
